...
/** vista que se pinta para enviar los parametros de busqueda por formulario via GET/POST a oaproductos */
	...
	if( !isset($avisoresultado) ) $avisoresultado = 'CONFIRMAR LOS DATOS Y EL AJUSTE';
	echo '<div>'.$avisoresultado.'</div>';

	$htmlformaattributos = array('name'=>'formulariofiltros','onSubmit'=>'return validageneric(this);');
	echo form_open_multipart('mproductos/oajusteforma/ajuste2creado', $htmlformaattributos) . PHP_EOL;
	...
	if( !isset($cod_tipo_list) ) $cod_tipo = array();
	...
	if( !isset($list_ajustar) ) $list_ajustar = '';
		...
		echo form_submit('but_proceso2', 'A_Procesar_2', 'class="btn-primary btn"');
